...
| `sig-reliability-plasticity-tension` | training | The open problem named as the field's central conflict: agents must be simultaneously **reliable** and **plastic**, and these are inherently opposed — "reliable systems resist change; plastic systems like change." Offered with an existence proof rather than a solution ("we are incredibly plastic but manage to be dependable most of the time") and a technical bet that **both parametric and non-parametric learning are needed**, with the synergy between them unsolved. **HELD PATTERN-LESS** — the stability/plasticity leg of the `pat-continual-learning-turn` ledger; note this is the same tension Asawa's benchmark operationalizes independently in this batch | — (held pattern-less) | `OnElement → el-continual-learning-definition`, `el-continual-learning` **[registry, b8]** |
