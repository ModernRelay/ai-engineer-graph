...
1. **⚑ RESOLVED — this is `pat-agent-memory-layer`'s day-one counter-edge.** The pattern was coined at review 2026-08-14 (defined in the Khemani file), and `sig-memory-adds-nothing-when-context-fits` now carries `ContradictsPattern` against it as intended — a controlled ablation showing that below the context-window threshold memory costs tokens and buys nothing. A pattern that ships with its own counter-evidence is exactly the shape a healthy one should have. Paired with Asawa's finding that vanilla in-context learning tops the continual-learning leaderboard, the batch supplies two independent, non-commercial results arguing that memory machinery is often unnecessary. Do not let the ten-talk track's weight bury them.
